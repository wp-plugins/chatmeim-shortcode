...
function userStatus_short($atts)
	{	
		...
		if ($hosted == "1") {
			return  '<img src="http://webchat.domains/status/' . $user . '" border="0" alt="Status">' . $link;
		} else {
			return '<img src="http://webchat.chatme.im/status/' . $user . '" border="0" alt="Status">' . $link;		
		}
	}		
...
function swatchTime_short()
	{	
		return "Internet Swatch Time <strong>@<!--mfunc echo date(B) -->" . date('B') . "<!--/mfunc--></strong>";
	}			
...
add_shortcode( 'swatchTime', 'swatchTime_short' );
?>